...
\n\n 
 `;
    ...
    context = context.replace("{{userDetails}}", JSON.stringify(userDetails, null, " "));
    ...
    if (response.data && response.data.choices && response.data.choices.length > 0 && response.data.choices[0].text) return response.data.choices[0].text.split('\n').join(" ");
    else return '';
}